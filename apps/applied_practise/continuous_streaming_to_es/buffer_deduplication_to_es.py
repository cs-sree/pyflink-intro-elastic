...
url_buffer = {}
status_buffer = {}
# only for unique doc_ids increase batch size
BATCH_SIZE = 3
...
def upsert_url(row):
    ...
    if len(url_buffer) >= BATCH_SIZE:
        ...
        print("[URL] Buffer not full yet, waiting for more data...")
...
def hello_pyflink():
    ...
    # State retention (TTL)
    t_env.get_config().get_configuration().set_string("table.exec.state.ttl", "20 s")
