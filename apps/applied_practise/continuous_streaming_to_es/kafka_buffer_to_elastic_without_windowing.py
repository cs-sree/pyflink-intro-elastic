...
def upsert_url(row):
    ...
    try:
        existing = es.get(index=URL_INDEX, id=doc_id)
        existing_count = existing['_source'].get('url_count', 0)
    except:
        existing_count = 0
...
def upsert_status(row):
    ...
    try:
        existing = es.get(index=STATUS_INDEX, id=doc_id)
        existing_count = existing['_source'].get('status_count', 0)
    except:
        existing_count = 0
...
def hello_pyflink():
    ...
    # s_env.enable_checkpointing(60000)  # every 60 seconds
    # s_env.get_checkpoint_config().set_checkpoint_storage("file:///tmp/flink-checkpoints")
    ...
    # Reading directly without windowing; all logic controlled via timing
    table_url = (
        t_env.from_path("kafka_table")
        .group_by(col("OrgUID"), col("ClientRequestURI"))
        .select(
            col("OrgUID"),
            col("ClientRequestURI").alias("URL"),
            col("ClientRequestURI").count.alias("URLCount"),
            col("EventTime").min.alias("WindowStart")   # ✅ Aggregate function required
)
    ...
    table_status = (
        t_env.from_path("kafka_table")
        .group_by(col("OrgUID"), col("EdgeResponseStatus"))
        .select(
            col("OrgUID"),
            col("EdgeResponseStatus").alias("Status"),
            col("EdgeResponseStatus").count.alias("StatusCount"),
            col("EventTime").min.alias("WindowStart")   
        )
    )

    # table_status.execute().print()
    ...
    # # Final flush before exit (useful for testing); assumes external 10-min trigger control
    # flush_to_es(URL_INDEX, url_buffer, lambda d: d)
    # flush_to_es(STATUS_INDEX, status_buffer, lambda d: d)
